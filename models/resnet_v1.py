...
class AdaNI_Basicblock_V1(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(AdaNI_Basicblock_V1, self).__init__()
    ...
    def forward(self, x):
        ...
        if out.shape[-1] == 32:
            saveF = out[0,0,:,:].detach().cpu().numpy()
            saveF = (saveF-saveF.min())/(saveF.max()-saveF.min())*256
            cv2.imwrite('output/fea1/'+'2_'+'816141'+'_before.jpg', saveF)
        ...
        if self.add_noise:
            ...
            # out += self.final_noise*10
            outweight = self.outweight(out)
            out = (self.final_noise+1)*outweight

        return F.relu(out, inplace=True)


class AdaNI_ResNet_V1(nn.Module):

    def __init__(self, block, depth, num_classes=10):

        super(AdaNI_ResNet_V1, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.add_noise:
            ...
            xweight = self.xweight(x)
            x = self.cn1_final_noise*xweight+ xweight
...
def adani_resnet_v1(depth = 20, num_classes=10):
    model = AdaNI_ResNet_V1(AdaNI_Basicblock_V1, depth=depth, num_classes=num_classes)
    return model
